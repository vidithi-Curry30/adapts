...
Figure~\ref{fig:optimization} shows the results of hyperparameter tuning for the BaselineFM and AdapTS system. The foundation model training loss converges smoothly over 50 epochs, reaching a final loss of 0.042. The hyperparameter sensitivity analysis reveals that:

\begin{itemize}
    \item Window size $W=5$ provides the best balance between adaptability and stability.
    \item Ridge regularization $\lambda=0.1$ consistently performs well across different window sizes.
    \item The full AdapTS system achieves 86.3\% MAE reduction compared to the FM + Conformal baseline on test datasets.
\end{itemize}

\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\textwidth]{optimization_focused_results.png}
    \caption{Hyperparameter tuning results. Top left: Foundation model training loss converges smoothly. Top right: Sensitivity analysis showing window size and regularization effects. Bottom left: Comparison of training vs. unseen test datasets. Bottom right: AdapTS vs. baseline performance, demonstrating 86.3\% MAE reduction.}
